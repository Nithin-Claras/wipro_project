...
int read_fn(FILE *input_file, char lines[][MAX_LINE_LENGTH]) {
    int line_count = 0;
    while (fgets(lines[line_count], MAX_LINE_LENGTH, input_file) != NULL) {
        line_count++;
...
void write_fn(FILE *output_file, char lines[][MAX_LINE_LENGTH], int line_count) {
